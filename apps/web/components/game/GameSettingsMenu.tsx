...
export default function GameSettingsMenu({
  ...
  return (
    <>
      ...
      {open ? (
        <div className="game-overlay-backdrop" onMouseDown={(event) => {
          ...
          >
            <div className="game-overlay-header">
              ...
              <button type="button" className="game-overlay-close" aria-label="Close game settings" onClick={close}>
                ✕
              </button>
            </div>
            ...
            <section className="game-settings-section">
              ...
              {volumeRows.map((row) => (
                <label key={row.label} className="game-settings-range font-ui">
                  <span>{row.label}<strong>{Math.round(row.value * 100)}%</strong></span>
                  <input
                    type="range"
                    min="0"
                    max="100"
